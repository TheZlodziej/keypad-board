...
public:
    AnalogInput(const AnalogInputConfig &config) : m_config(config), m_last_changed(config.debounce_time)
    {
    ...
    void update()
    {
        ...
        const float new_value = static_cast<float>(raw_reading) / ((1 << m_config.adc_resolution) - 1);
